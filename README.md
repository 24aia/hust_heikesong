...
- 顶部仅显示当前断点类型；点击标题切换至另一种已存在的记录。手动书签优先、自动保存不抢走选择的规则保持不变。
- 点击“解释词句”，在已加载正文中拖选一个单词或句子，再确认“开始解释”；支持语境释义、翻译和继续追问。普通划词不弹出入口、不发请求。解释面板返回后对话丢弃，不保存书签备注。
- 刘看山平时瞌睡，打开时打招呼，选择书签时运球，等待解释或回顾时使用电脑，完成后待机。按用户选择播放伙伴动画；收起伙伴或页面在后台时使用静态形象。
- 解释与回顾独立：解释只传所选词句、附近原文和本次对话；回顾依然严格排除断点段落及后文。解释不承诺文献检索或事实核查。

以上改动需使用重新构建的扩展，文首链接的历史签名包不会自动包含这些更新。
