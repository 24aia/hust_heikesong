...
- 回顾功能直接调用知乎直答 API。只有用户主动点击回顾时，断点之前的正文与页面标题才会被发送；发布包若内置有效凭据即可直接使用。
